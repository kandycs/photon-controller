...
/**
 * This class implements a Xenon service representing a task to expand a
 * cluster.
 */
public class ClusterExpandTaskService extends StatefulService {
  ...
  private void initializeExpandCluster(final State currentState, final ClusterService.State clusterDocument)
      throws IOException {

    HostUtils.getApiClient(this).getClusterApi().getVmsInClusterAsync(currentState.clusterId,
        new FutureCallback<ResourceList<Vm>>() {
          ...
          public void onSuccess(@Nullable ResourceList<Vm> result) {
            ...
            case SWARM:
              ...
              break;
            case GRIDENGINE:
              masterNodeTag = ClusterUtil.createClusterNodeTag(currentState.clusterId, NodeType.GridengineMaster);
              slaveNodeTag = ClusterUtil.createClusterNodeTag(currentState.clusterId, NodeType.GridengineSlave);
              break;
            default:
  ...
  private void getMasterIp(final State currentState, final ClusterService.State clusterDocument,
      final int slaveCountDelta, final String masterVmId) {
    WaitForNetworkTaskService.State startState = new WaitForNetworkTaskService.State();
    startState.vmId = masterVmId;

    TaskUtils.startTaskAsync(this, WaitForNetworkTaskFactoryService.SELF_LINK, startState,
        state -> TaskUtils.finalTaskStages.contains(state.taskState.stage), WaitForNetworkTaskService.State.class,
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, new FutureCallback<WaitForNetworkTaskService.State>() {
          @Override
          public void onSuccess(@Nullable WaitForNetworkTaskService.State result) {
            ...
            case FINISHED:
              ...
              break;
            case CANCELLED:
              TaskUtils.sendSelfPatch(ClusterExpandTaskService.this, buildPatch(TaskState.TaskStage.CANCELLED));
              break;
            case FAILED:
  ...
  private void expandCluster(State currentState, ClusterService.State clusterDocument, int slaveCountDelta,
      String masterIp) {

    if (slaveCountDelta > 0) {
      ServiceUtils.logInfo(this,
          String.format("Expected slave count is %d, delta is %d", clusterDocument.slaveCount, slaveCountDelta));
      ...
      case KUBERNETES: {
        ...
        String cn = clusterDocument.extendedProperties.get(ClusterManagerConstants.EXTENDED_PROPERTY_CONTAINER_NETWORK);
      ...
      case GRIDENGINE: {
        String nfsIp = clusterDocument.extendedProperties.get(ClusterManagerConstants.EXTENDED_PROPERTY_NFS_IP);

        input.serverAddress = masterIp;
        input.nodeProperties = GridengineSlaveNodeTemplate.createProperties(nfsIp, masterIp);
        input.nodeType = NodeType.GridengineSlave;
        break;
      }
      default:
