...
/**
 * This class implements a Xenon service representing a task to inspect cluster
 * for inactive slave vms.
 */
public class GarbageInspectionTaskService extends StatefulService {
  ...
  private void getMasterIp(final State currentState, final ClusterService.State clusterState, final String masterVmId,
        ...
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, new FutureCallback<WaitForNetworkTaskService.State>() {
          ...
          public void onSuccess(@Nullable WaitForNetworkTaskService.State result) {
            ...
            case FINISHED:
              ...
              break;
            case CANCELLED:
              TaskUtils.sendSelfPatch(GarbageInspectionTaskService.this, buildPatch(TaskState.TaskStage.CANCELLED));
              break;
            case FAILED:
   ...
   * @param currentState
   */
  private void getSlavesFromMaster(final State currentState, final ClusterService.State clusterState,
      final String masterIp, final Set<Vm> allSlaves) {

    PhotonControllerXenonHost photonControllerXenonHost = (PhotonControllerXenonHost) getHost();
    ClusterManagerFactory clusterManagerFactory = ((ClusterManagerFactoryProvider) photonControllerXenonHost
        .getDeployer()).getClusterManagerFactory();
    ...
    case SWARM:
      ...
      break;
    case GRIDENGINE:
      checker = helper.createSlavesStatusChecker(this, NodeType.GridengineSlave);
      break;
    default:
      failTask(
          new UnsupportedOperationException("ClusterType is not supported. ClusterType: " + clusterState.clusterType));
      return;
    }
      ...
      public void onSuccess(@Nullable Set<String> activeNodes) {
        try {
          ...
          if (inactiveVms.size() > 0) {
            ...
            TaskUtils.sendSelfPatch(GarbageInspectionTaskService.this, buildPatch(TaskState.TaskStage.FINISHED));
          }
        } catch (Throwable t) {
          failTask(t);
        }
  ...
  private void createInactiveVmEntities(String clusterId, Set<String> inactiveVmIds) {
    ...
    OperationJoin.create(inactiveVmIds.stream().map((String vmId) -> {
