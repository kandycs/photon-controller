...
/**
 * This class implements a Xenon Service that performs periodic maintenance on a
 * single cluster.
 */
public class ClusterMaintenanceTaskService extends StatefulService {
  ...
  public void handlePatch(Operation patchOperation) {
    ...
    // Complete MUST be called only after we have decided to run maintenance.
    // This way
    // we avoid race conditions related to multiple Patch calls trying to start
    // maintenance.
    ...
    try {
      ...
      case RUN:
        ...
        break;
      case RETRY:
        getHost().schedule(() -> startMaintenance(currentState, clusterId),
            currentState.retryIntervalSecond * currentState.retryCount, TimeUnit.SECONDS);
        break;
      case SKIP:
        ServiceUtils.logInfo(this, "Skipping maintenance");
        break;
      default:
        ServiceUtils.logSevere(this, "Unknown maintenance operation %s. Skipping maintenance",
  ...
  private MaintenanceOperation processPatchState(State currentState, State patchState, String clusterId) {
    ...
    if (currentState.taskState.stage == TaskState.TaskStage.STARTED) {
      if (patchState.taskState.stage == TaskState.TaskStage.FINISHED) {
        // The previous maintenance task succeeded. We need to reset the retry
        // counter.
        ServiceUtils.logInfo(this, "Not run maintenance because patching the task from %s to %s",
            currentState.taskState.stage.toString(), patchState.taskState.stage.toString());
        ...
        // The previous maintenance task failed. We need to record the error
        // message and determine
        // whether we want to retry.
        patchState.errors = currentState.errors == null ? new ArrayList<>() : currentState.errors;
        ...
        if (currentState.retryCount < currentState.maxRetryCount) {
          ...
          ServiceUtils.logInfo(this, "Retry maintenance because current retry count is %d", currentState.retryCount);
          ...
          // We have used all retries. Fail the maintenance task and set the
          ...
          sendRequest(HostUtils.getCloudStoreHelper(this).createPatch(getClusterDocumentLink(clusterId))
              .setBody(clusterPatchState).setCompletion((Operation operation, Throwable throwable) -> {
                if (null != throwable) {
                  // Ignore the failure. Otherwise if we fail the maintenance
                  // task we may end up
                  // in a dead loop.
        ...
        // The previous maintenance task was cancelled. We don't want to run
        // maintenance task but we want
        ...
        sendRequest(HostUtils.getCloudStoreHelper(this).createPatch(getClusterDocumentLink(clusterId))
            .setBody(clusterPatchState).setCompletion((Operation operation, Throwable throwable) -> {
              if (null != throwable) {
                // Ignore the failure. Otherwise if we fail the maintenance task
                // we may end up
                // in a dead loop.
                ServiceUtils.logSevere(this, "Failed to patch cluster to ERROR: %s", throwable.toString());
              }
  ...
  private void startMaintenance(State currentState, String clusterId) {
    ...
    sendRequest(HostUtils.getCloudStoreHelper(this).createGet(getClusterDocumentLink(clusterId))
        .setCompletion((Operation op, Throwable t) -> {
          if (t != null) {
            if (op.getStatusCode() == Operation.STATUS_CODE_NOT_FOUND
                || op.getStatusCode() == Operation.STATUS_CODE_TIMEOUT || t.getClass().equals(TimeoutException.class)) {
              sendRequest(Operation.createDelete(UriUtils.buildUri(getHost(), getSelfLink()))
                  .setBody(new ServiceDocument()).setReferer(getHost().getUri()));
          ...
          try {
            ...
            default:
              failTask(new IllegalStateException(String.format("Unknown clusterState. ClusterId: %s. ClusterState: %s",
                  clusterId, cluster.clusterState)));
              break;
            }
          } catch (Throwable e) {
            failTask(e);
  ...
  private void performGarbageInspection(final State currentState, final String clusterId) {
    ...
    TaskUtils.startTaskAsync(this, GarbageInspectionTaskFactoryService.SELF_LINK, startState,
        state -> TaskUtils.finalTaskStages.contains(state.taskState.stage), GarbageInspectionTaskService.State.class,
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, new FutureCallback<GarbageInspectionTaskService.State>() {
          @Override
          public void onSuccess(@Nullable GarbageInspectionTaskService.State result) {
            ...
            case FINISHED:
              ...
              break;
            case CANCELLED:
              IllegalStateException cancelled = new IllegalStateException(
                  String.format("GarbageInspectionTaskService was canceled. %s", result.documentSelfLink));
            ...
            case FAILED:
              IllegalStateException failed = new IllegalStateException(
                  String.format("GarbageInspectionTaskService failed with error %s. %s",
  ...
  private void performGarbageCollection(final State currentState, final String clusterId) {
    ...
    TaskUtils.startTaskAsync(this, GarbageCollectionTaskFactoryService.SELF_LINK, startState,
        state -> TaskUtils.finalTaskStages.contains(state.taskState.stage), GarbageCollectionTaskService.State.class,
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, new FutureCallback<GarbageCollectionTaskService.State>() {
          @Override
          public void onSuccess(@Nullable GarbageCollectionTaskService.State result) {
            ...
            case CANCELLED:
              IllegalStateException cancelled = new IllegalStateException(
                  String.format("GarbageCollectionTaskService was canceled. %s", result.documentSelfLink));
              TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this,
                  buildPatch(TaskState.TaskStage.CANCELLED, cancelled));
              break;
            case FAILED:
              IllegalStateException failed = new IllegalStateException(
                  String.format("GarbageCollectionTaskService failed with error %s. %s",
                      result.taskState.failure.message, result.documentSelfLink));
              TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this,
                  buildPatch(TaskState.TaskStage.FAILED, failed));
              break;
  ...
  private void expandCluster(final State currentState, final String clusterId) {
    ...
    TaskUtils.startTaskAsync(this, ClusterExpandTaskFactoryService.SELF_LINK, startState,
        state -> TaskUtils.finalTaskStages.contains(state.taskState.stage), ClusterExpandTaskService.State.class,
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, new FutureCallback<ClusterExpandTaskService.State>() {
          @Override
          public void onSuccess(@Nullable ClusterExpandTaskService.State result) {
            ...
            case CANCELLED:
              IllegalStateException cancelled = new IllegalStateException(
                  String.format("ClusterExpandTaskService was canceled. %s", result.documentSelfLink));
              TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this,
                  buildPatch(TaskState.TaskStage.CANCELLED, cancelled));
              break;
            case FAILED:
              IllegalStateException failed = new IllegalStateException(
                  String.format("ClusterExpandTaskService failed with error %s. %s", result.taskState.failure.message,
  ...
  private void deleteCluster(String clusterId) {
    ...
    FutureCallback<ClusterDeleteTask> callback = new FutureCallback<ClusterDeleteTask>() {
      ...
      public void onSuccess(@Nullable ClusterDeleteTask result) {
        switch (result.taskState.stage) {
        case FINISHED:
          TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this, buildPatch(TaskState.TaskStage.FINISHED, null));
          break;
        case CANCELLED:
          IllegalStateException cancelled = new IllegalStateException(
              String.format("ClusterDeleteTaskService was canceled. %s", result.documentSelfLink));
          TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this,
              buildPatch(TaskState.TaskStage.CANCELLED, cancelled));
          break;
        case FAILED:
          IllegalStateException failed = new IllegalStateException(
              String.format("ClusterDeleteTaskService failed with error %s. %s", result.taskState.failure.message,
                  result.documentSelfLink));
          TaskUtils.sendSelfPatch(ClusterMaintenanceTaskService.this, buildPatch(TaskState.TaskStage.FAILED, failed));
          break;
        }
      }
    ...
    TaskUtils.startTaskAsync(this, ClusterDeleteTaskFactoryService.SELF_LINK, startState,
        (ClusterDeleteTask state) -> TaskUtils.finalTaskStages.contains(state.taskState.stage), ClusterDeleteTask.class,
        ClusterManagerConstants.DEFAULT_TASK_POLL_DELAY, callback);
  }
  ...
  private void validateStartState(State state) {
    ...
    if (state.taskState.stage == TaskState.TaskStage.STARTED) {
      throw new IllegalStateException(
          "Cannot create a maintenance task in STARTED state. " + "Use PATCH method to set the task state to STARTED.");
    }
  }
  ...
  private void validatePatchState(State startState, State patchState) {
    ...
    // This task is restartable from any of the terminal states. Hence, if the
    // patch is to START the task,
  ...
  private void failTask(Throwable t) {
    ServiceUtils.logSevere(this, "Cluster Maintenance failed. SelfLink: %s. Error: %s", getSelfLink(), t.toString());
    TaskUtils.sendSelfPatch(this, buildPatch(TaskState.TaskStage.FAILED, t));
  }

  private void updateStates(String clusterId, ClusterService.State clusterPatch, State patch) {

    sendRequest(HostUtils.getCloudStoreHelper(this).createPatch(getClusterDocumentLink(clusterId)).setBody(clusterPatch)
        .setCompletion((Operation operation, Throwable throwable) -> {
          if (null != throwable) {
            // Ignore the failure. Otherwise if we fail the maintenance task may
            // end up
            // in a dead loop.
  ...
  public static class State extends ServiceDocument {
    ...
    /**
     * This value represents a list of errors that the maintenance task has
     * encountered.
     */
    ...
    /**
     * This value represents the maximum number of retries that maintenance task
     * can take before entering FAILED state.
     */
    ...
    /**
     * This value represents the number of available retries that maintenance
     * task has taken before entering FAILED state.
     */
    @DefaultInteger(value = 0)
    public Integer retryCount;

    /**
     * This value represents the retry interval, in seconds, that maintenance
     * task should wait before retry again.
     */
  ...
  private static enum MaintenanceOperation {
    ...
    SKIP,
    // RETRY means that we run maintenance task in this cycle but with the retry
    // delay.
    RETRY
  }
}
