...
import com.vmware.photon.controller.clustermanager.servicedocuments.GridengineClusterCreateTask; //hacked by chaoc
...
/**
 * Cluster Manager Client Facade that exposes cluster manager functionality via
 * high-level methods, and hides Xenon protocol details.
 */
@Singleton
public class ClusterManagerClient {
  ...
  public static final String EXTENDED_PROPERTY_NFS_IP = "nfs_ip"; // hacked by
                                                                  // chaoc
  ...
  public GridengineClusterCreateTask createGridengineCluster(String projectId, ClusterCreateSpec spec)
      throws SpecInvalidException {
    ClusterConfigurationService.State clusterConfiguration = getClusterConfiguration(ClusterType.GRIDENGINE);
    String clusterId = createGridengineClusterEntity(projectId, spec, clusterConfiguration);
    GridengineClusterCreateTask createTask = new GridengineClusterCreateTask();
    createTask.clusterId = clusterId;
    createTask.slaveBatchExpansionSize = spec.getSlaveBatchExpansionSize() == 0 ? null
        : spec.getSlaveBatchExpansionSize();

    // Post createTask to GridengineClusterCreateTaskService
    Operation operation = xenonClient.post(ServiceUriPaths.GRIDENGINE_CLUSTER_CREATE_TASK_SERVICE, createTask);
    return operation.getBody(GridengineClusterCreateTask.class);
  }

  public GridengineClusterCreateTask getGridengineClusterCreationStatus(String creationTaskLink)
  ...
  public KubernetesClusterCreateTask createKubernetesCluster(String projectId, ClusterCreateSpec spec)
    ...
    Operation operation = xenonClient.post(ServiceUriPaths.KUBERNETES_CLUSTER_CREATE_TASK_SERVICE, createTask);
  ...
  public MesosClusterCreateTask createMesosCluster(String projectId, ClusterCreateSpec spec)
    ...
    Operation operation = xenonClient.post(ServiceUriPaths.MESOS_CLUSTER_CREATE_TASK_SERVICE, createTask);
  ...
  public SwarmClusterCreateTask createSwarmCluster(String projectId, ClusterCreateSpec spec)
    ...
    Operation operation = xenonClient.post(ServiceUriPaths.SWARM_CLUSTER_CREATE_TASK_SERVICE, createTask);
  ...
  public ClusterResizeTask resizeCluster(String clusterId, ClusterResizeOperation resizeOperation) {
    ...
    Operation operation = xenonClient.post(ServiceUriPaths.CLUSTER_RESIZE_TASK_SERVICE, resizeTask);
    return operation.getBody(ClusterResizeTask.class);
  }

  public ClusterResizeTask getClusterResizeStatus(String resizeTaskLink) throws DocumentNotFoundException {
  ...
  public ClusterDeleteTask deleteCluster(String clusterId) {
    ...
    Operation operation = xenonClient.post(ServiceUriPaths.CLUSTER_DELETE_TASK_SERVICE, deleteTask);

    return operation.getBody(ClusterDeleteTask.class);
  }

  public ClusterDeleteTask getClusterDeletionStatus(String deletionTaskLink) throws DocumentNotFoundException {
  ...
  public ResourceList<Cluster> getClusters(String projectId, Optional<Integer> pageSize) throws ExternalException {
    ServiceDocumentQueryResult queryResult = apiFeXenonClient.queryDocuments(ClusterService.State.class,
        ImmutableMap.of("projectId", projectId), pageSize, true);
  ...
  private ClusterConfigurationService.State getClusterConfiguration(ClusterType clusterType)
      throws SpecInvalidException {
    List<ClusterConfigurationService.State> configurations = apiFeXenonClient
        .queryDocuments(ClusterConfigurationService.State.class, ImmutableMap.of("documentSelfLink",
  ...
  private ClusterService.State assembleCommonClusterEntity(ClusterType clusterType, String projectId,
      ClusterCreateSpec spec, ClusterConfigurationService.State clusterConfiguration) throws SpecInvalidException {
  ...
  private String createGridengineClusterEntity(String projectId, ClusterCreateSpec spec,
      ClusterConfigurationService.State clusterConfiguration) throws SpecInvalidException {
    String nfsIp = spec.getExtendedProperties().get(EXTENDED_PROPERTY_NFS_IP);
    if (nfsIp == null) {
      throw new SpecInvalidException("Missing extended property: NFS IP");
    }
    if (!InetAddressValidator.getInstance().isValidInet4Address(nfsIp)) {
      throw new SpecInvalidException("Invalid extended property: nfs ip: " + nfsIp);
    }
    ...
    ClusterService.State cluster = assembleCommonClusterEntity(ClusterType.GRIDENGINE, projectId, spec,
        clusterConfiguration);
    cluster.extendedProperties.put(ClusterManagerConstants.EXTENDED_PROPERTY_NFS_IP, nfsIp);
