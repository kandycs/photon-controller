...
import com.vmware.photon.controller.apife.commands.steps.GridengineClusterCreateStepCmd;
...
public class ClusterBackend {
  ...
  @Inject
  public ClusterBackend(ClusterManagerClient clusterManagerClient, TaskBackend taskBackend, VmBackend vmBackend)
  ...
  public TaskEntity create(String projectId, ClusterCreateSpec spec)
    ...
    case GRIDENGINE:
      return createGridengineCluster(projectId, spec);
    default:
  ...
  public TaskEntity resize(String clusterId, ClusterResizeOperation resizeOperation)
    ...
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        Operation.RESIZE_CLUSTER_INITIATE);
  ...
  public TaskEntity delete(String clusterId) throws TaskNotFoundException, ClusterNotFoundException {
    ...
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        Operation.DELETE_CLUSTER_INITIATE);
  ...
  private TaskEntity createKubernetesCluster(String projectId, ClusterCreateSpec spec)
    ...
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        Operation.CREATE_KUBERNETES_CLUSTER_INITIATE);
  ...
  // hacked by Chaoc, adding a new cluster type, GRIDENGINE
  private TaskEntity createGridengineCluster(String projectId, ClusterCreateSpec spec)
      throws SpecInvalidException, TaskNotFoundException {
    // Create the steps
    TaskEntity taskEntity = taskBackend.createQueuedTask(null, Operation.CREATE_CLUSTER);
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        Operation.CREATE_GRIDENGINE_CLUSTER_INITIATE);

    // Pass projectId and createSpec to initiateStep as transient resources
    initiateStep.createOrUpdateTransientResource(GridengineClusterCreateStepCmd.PROJECT_ID_RESOURCE_KEY, projectId);
    initiateStep.createOrUpdateTransientResource(GridengineClusterCreateStepCmd.CREATE_SPEC_RESOURCE_KEY, spec);

    // Dummy steps that are mapped to GridengineClusterCreateTask's subStages
    taskBackend.getStepBackend().createQueuedStep(taskEntity, Operation.CREATE_GRIDENGINE_CLUSTER_SETUP_NFS);
    taskBackend.getStepBackend().createQueuedStep(taskEntity, Operation.CREATE_GRIDENGINE_CLUSTER_SETUP_MASTER);
    taskBackend.getStepBackend().createQueuedStep(taskEntity, Operation.CREATE_GRIDENGINE_CLUSTER_SETUP_SLAVES);
  ...
  private TaskEntity createMesosCluster(String projectId, ClusterCreateSpec spec)
    ...
    TaskEntity taskEntity = taskBackend.createQueuedTask(null, Operation.CREATE_CLUSTER);
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        Operation.CREATE_MESOS_CLUSTER_INITIATE);
  ...
  private TaskEntity createSwarmCluster(String projectId, ClusterCreateSpec spec)
      throws SpecInvalidException, TaskNotFoundException {
    // Create the steps
    TaskEntity taskEntity = taskBackend.createQueuedTask(null,
        com.vmware.photon.controller.api.Operation.CREATE_CLUSTER);
    StepEntity initiateStep = taskBackend.getStepBackend().createQueuedStep(taskEntity,
        com.vmware.photon.controller.api.Operation.CREATE_SWARM_CLUSTER_INITIATE);
